...
- `segment` : `gites`, `wedding_planner`, `lieu_reception`, `entreprise`, `conciergerie`, `cave`, `particulier`, `particulier_plateforme`.
- `id` des leads entrants : `IN-AAAAMMJJ-NN`.
...
Leads de plateformes (ChefMaison, StarOfService…) : `a_repondre_plateforme` → `repondu` → … (la réponse se fait sur la plateforme, pas par Gmail).
